...
async function main() {
  ...
  const action: "deposit" | "withdraw" = "withdraw";
...
async function performWithdraw(wallet: ethers.Wallet, contract: ethers.Contract, amount: string) {
  ...
  const tx = await contract.withdraw!(handleAsBytes, eWithdrawAmount.inputProof);
  ...
  try {
    ...
    console.log("⏳ Waiting for withdrawal to be fully processed...");
    await new Promise((resolve) => setTimeout(resolve, 5000));
  } catch (error: any) {
    console.error("❌ Error during off-chain decryption or withdrawal completion:", error.message);
    console.log(
      "\n⚠️  Withdrawal was initiated but not completed. You can retry by calling completeWithdrawal() manually with the handle:",
      handle,
    );
  }
}
